...
public class WebhookRepository implements WebhookGateway {
	...
	public Pagamento atualizaPagamento(Pagamento pagamento) throws Exception{
		MAPPER.typeMap(PagamentoEntity.class, Pagamento.class)
				...
				.addMappings(mapperB -> mapperB.map(src -> src.getPedido().getId(), Pagamento::setNumeroPedido))
				.addMappings(mapperC -> {
		...
		pagamentoEntity = pagamentoJpaRepository.findById(pagamento.getNumeroPagamento()).map(pagEntity -> {
			pagEntity.setDataPagamento(dataPagamento);
			pagEntity.setStatusPagamento(statusPagamento);
			pagEntity.getHistoricoPagamento().add(HistoricoPagamentoEntity.adicionaHistorico(descricaoHistorico,
					pagEntity, dataPagamento, tentativas));
